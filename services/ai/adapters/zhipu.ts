...
// Zhipu GLM — Chinese AI company's GLM series models
export function createZhipuAdapter(apiKey: string): AIProviderAdapter {
  const BASE = 'https://open.bigmodel.cn/api/paas/v4/chat/completions';

  const MODEL_MAP: Record<string, string> = {
    classify:  'glm-4-flash',
    summarize: 'glm-4-plus',
    extract:   'glm-4-flash',
    generate:  'glm-4-plus',
    score:     'glm-4-flash',
  };
  ...
  return {
    ...
    async complete(request: AIRequest): Promise<AIResponse> {
      ...
      if (!res.ok) throw new Error(`Zhipu error ${res.status}`);
    ...
    async *completeStream(request: AIRequest): AsyncGenerator<string, AIResponse> {
      ...
      if (!res.ok) throw new Error(`Zhipu stream error ${res.status}`);
