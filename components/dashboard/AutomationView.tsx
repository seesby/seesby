    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => setIsAddOpen(true)} className="bg-white text-black px-4 py-2 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors flex items-center gap-2">
            ...
            {rules.length === 0 ? (
                <div className="p-12 text-center bg-[#0F0F0F] rounded-3xl border border-white/5 border-dashed">
                    ...
                    <p className="text-gray-500 mb-4 max-w-md mx-auto">Set up rules to get notified or take action automatically when something changes in your SEO data.</p>
                    <button onClick={() => setIsAddOpen(true)} className="bg-white/5 hover:bg-white/10 border border-white/10 text-white px-6 py-2 rounded-xl text-sm font-bold transition-colors">
                ...
                <div className="grid grid-cols-1 gap-4">
                    {rules.map((rule) => (
                        <div key={rule.id} className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-6 flex items-center justify-between group hover:border-white/10 transition-all">
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${rule.is_active ? 'bg-brand-green/10 text-brand-green' : 'bg-gray-800 text-gray-500'}`}>
                                    <Zap size={20} />
                                </div>
                                <div>
                                    <h3 className="font-bold text-white">{rule.name}</h3>
                                    <div className="flex items-center gap-2 mt-1 text-xs text-gray-400">
                                        <span>If: <strong className="text-gray-300">{rule.trigger_condition}</strong></span>
                                        <ArrowRight size={10} />
                                        <span>Then: <strong className="text-gray-300">{rule.action}</strong></span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <button onClick={() => deleteRule(rule.id)} className="text-gray-600 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                                    <Trash2 size={16} />
                                </button>
                                <span className={`text-xs font-bold uppercase ${rule.is_active ? 'text-brand-green' : 'text-gray-500'}`}>
                                ...
                                >
                                    <div className={`w-4 h-4 bg-white rounded-full transition-transform ${rule.is_active ? 'translate-x-4' : ''}`}></div>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center backdrop-blur-sm p-4">
                    <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
                        <div className="p-5 border-b border-white/5 flex justify-between items-center bg-[#151515]">
                            ...
                            <button onClick={() => setIsAddOpen(false)} className="text-gray-500 hover:text-white"><X size={20} /></button>
                        </div>
