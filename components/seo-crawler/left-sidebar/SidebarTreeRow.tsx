		...
		<>
			<button
				...
				className={`w-full h-[26px] flex items-center gap-1 pr-3 rounded-sm text-[11px] font-medium transition-colors
					${selected ? 'bg-[#F59E0B]/10 text-[var(--brand-text-strong)]' : isAction ? 'text-[#66666E] hover:text-[var(--brand-text-strong)]' : 'text-[#A0A0A5] hover:bg-[var(--brand-surface-3)]/[0.03] hover:text-[var(--brand-text-strong)]'}`}
			>
				<div className="w-4 shrink-0 flex items-center justify-start">
					{item.children ? (
						<span className={`text-[14px] ${selected ? 'text-[var(--brand-text-strong)]' : 'text-[#55555A]'}`}>▸</span>
					) : Icon ? (
						...
						<span className={`text-[10px] ${selected ? 'text-[var(--brand-text-strong)]' : tone ? tone.text : 'text-[#55555A]'}`}>
							{item.bullet}
						</span>
					) : section.bullet === 'arrow' ? (
						<span className={`text-[14px] ${selected ? 'text-[var(--brand-text-strong)]' : tone ? tone.text : 'text-[#55555A]'}`}>▸</span>
					) : section.bullet === 'dot' ? (
						<span className={`text-[10px] ${selected ? 'text-[var(--brand-text-strong)]' : tone ? tone.text : 'text-[#55555A]'}`}>●</span>
					) : section.bullet === 'dot-filled' ? (
						<span className={`text-[12px] ${selected ? 'text-[var(--brand-text-strong)]' : tone ? tone.text : 'text-[#55555A]'}`}>⬤</span>
				...
				{item.meta && (
					<span className={`text-[10px] font-mono tabular-nums ${isAction || item.metaStyle === 'badge' ? 'px-1 py-0.5 rounded-full bg-[var(--brand-surface-4)] text-[var(--brand-text-strong)]' : item.meta.includes('●') || item.meta.includes('⬤') ? (tone ? tone.text : 'text-[#66666E]') : tone ? tone.text : 'text-[#66666E]'}`}>
						{item.meta}
					</span>
				)}
				{item.indicator === 'dot' && (
