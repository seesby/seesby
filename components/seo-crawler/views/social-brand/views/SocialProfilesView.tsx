...
function ProfileCard({ profile: p }: { profile: any }) {
  ...
  const growthColor = growthRate > 0.01 ? STATUS.good : growthRate < 0 ? STATUS.bad : 'text-[var(--brand-text-mid)]';
  const completenessColor = completeness >= 90 ? STATUS.good : completeness >= 70 ? '#f59e0b' : STATUS.bad;
  const engColor = engRate > 0.05 ? STATUS.good : engRate > 0.02 ? 'text-[var(--brand-text-mid)]' : 'text-[var(--brand-text-faint)]';
    ...
    <article className={`${CARD} flex flex-col`}>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-right">
          <div className="text-[16px] font-mono text-[var(--brand-text-strong)] leading-none">{fmtCompact(p.followers ?? 0)}</div>
        </div>
      </div>
...
function NetworkIcon({ network }: { network: string }) {
  ...
  return (
    <div
      className="w-7 h-7 rounded flex items-center justify-center text-[9px] font-bold text-[var(--brand-text-strong)] shrink-0"
      style={{ background: colors[network] ?? 'bg-[var(--brand-surface-4)]' }}
    >
