...
export function useOrphansDepth() {
  ...
  const columns: ColumnDef<OrphanRow>[] = [
    ...
    { accessorKey: 'inSitemap', header: 'Sitemap', size: 80, cell: c => c.getValue() ? <span className="text-[#22c55e]">yes</span> : <span className="text-[#666]">no</span> },
    { accessorKey: 'template', header: 'Template', size: 120, cell: c => <span className="text-[#888] truncate max-w-[100px]">{c.getValue() as string}</span> },
  ];
