...
const PANEL = 'rounded border border-[#1a1a1a] bg-[#0a0a0a]/90 backdrop-blur';
const LABEL = 'text-[10px] uppercase tracking-wider text-[#666] mb-1';
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 relative overflow-hidden">
      ...
      <div className="absolute top-2 left-2 z-10 flex flex-col gap-1.5">
        <div className="flex items-center gap-1 bg-[#0c0c0c]/90 backdrop-blur rounded border border-[#1a1a1a] p-0.5">
          {SCOPE_OPTIONS.map(opt => (
            <button
              ...
              className={clsx(
                ...
                scope === opt.id ? 'bg-[#171717] text-white' : 'text-[#888] hover:text-[#ddd]'
              )}
            >
              {opt.label}
            </button>
        ...
        <div className="flex items-center gap-1 bg-[#0c0c0c]/90 backdrop-blur rounded border border-[#1a1a1a] p-0.5">
          {COLOR_OPTIONS.map(opt => (
            <button
              ...
              className={clsx(
                'h-[24px] px-2 text-[10px] rounded transition-colors',
                colorBy === opt.value ? 'bg-[#171717] text-white' : 'text-[#888] hover:text-[#ddd]'
              )}
            >
              {opt.label}
            </button>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex gap-2 p-2 pointer-events-none">
        ...
        <div className={`${PANEL} flex-1 pointer-events-auto`}>
          <div className="px-2.5 pt-1.5 pb-2">
            ...
            <div className="flex h-[8px] rounded-full overflow-hidden bg-[#171717]">
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5 text-[9px]">
              {buckets.filter(b => b.count > 0).map(b => (
                <span key={b.type} className="flex items-center gap-1">
                  ...
                  <span className="text-[#888]">{b.type}</span>
                  <span className="text-[#bbb] font-mono">{b.percentage}%</span>
                </span>
              ))}
            </div>
          </div>
      ...
      <div className="absolute bottom-[90px] left-2 z-10 flex items-center gap-2 bg-[#0c0c0c]/90 backdrop-blur rounded border border-[#1a1a1a] px-3 py-1.5 text-[10px]">
      ...
      {/* Bottom stats */}
      <div className="absolute bottom-[90px] right-2 z-10 flex items-center gap-4 bg-[#0c0c0c]/90 backdrop-blur rounded border border-[#1a1a1a] px-3 py-1.5 text-[10px]">
        <span className="text-[#888]">{filteredNodes.length} nodes</span>
        <span className="text-[#888]">{filteredLinks.length} links</span>
        <span className="text-[#888]">Internal: <span className="text-[#a78bfa]">{stats.internalCount}</span></span>
        <span className="text-[#888]">External: <span className="text-[#14b8a6]">{stats.externalCount}</span></span>
        <span className="text-[#888]">Max depth: <span className="text-[#f59e0b]">{stats.maxDepth}</span></span>
      </div>
    </div>
  );
}
