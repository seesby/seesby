  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-auto custom-scrollbar">
      ...
      <div className="flex items-center gap-4 p-3 pb-0">
          ...
          {(['internal', 'external'] as const).map(s => (
            <button
              ...
              className={clsx(
                ...
                scope === s ? 'bg-[#171717] text-white' : 'text-[#666] hover:text-[#999]'
              )}
            >
              {s}
            </button>
        ...
        <div className="flex items-center gap-1">
          <span className="text-[10px] text-[#666] mr-1">Group:</span>
          {GROUP_FILTERS.map(f => (
            <button
              key={f.id}
              ...
              className={clsx(
                'h-[22px] px-2 text-[10px] rounded transition-colors',
                group === f.id ? 'bg-[#171717] text-white' : 'text-[#666] hover:text-[#999]'
              )}
            >
              {f.label}
            </button>
        ...
        <div className={`${PANEL} col-span-12`}>
          ...
          <div className="px-3 pb-3">
            {filtered.length > 0 ? (
              ...
              <div className="text-[11px] text-[#555] py-6 text-center">No anchor data</div>
            )}
          </div>
        </div>
              ...
              <tbody>
                {targetMix.length > 0 ? targetMix.map((t, i) => (
                  <tr key={i} className="border-b border-[#111]">
                    <td className="py-1 pr-4 text-[#bbb] truncate max-w-[200px]">{shortUrl(t.target)}</td>
                    ...
                    <td className="py-1 px-2 text-right font-mono text-[#666]">{t.generic}%</td>
                    <td className="py-1 px-2 text-right font-mono text-[#666]">{t.naked}%</td>
                    <td className="py-1 pl-2 text-right font-mono text-[#666]">{t.image}%</td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan={7} className="py-4 text-center text-[#555]">No anchor data available</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div className="px-3 pb-3 space-y-1">
              {overOptimized.map((o, i) => (
                <div key={i} className="flex items-center gap-2 text-[10px]">
                  <span className="text-[#bbb]">{shortUrl(o.target)}</span>
...
function AnchorBar({ item, max }: { item: AnchorPhraseWithClass; max: number }) {
  ...
  return (
    <div className="flex items-center gap-2 group">
      <span className="text-[10px] text-[#bbb] w-[140px] truncate shrink-0" title={item.name}>
        {item.name || '(empty)'}
      </span>
      ...
      <span className="text-[10px] font-mono text-[#888] w-[40px] text-right shrink-0 tabular-nums">
        {item.value}
      </span>
      {item.warning && (
        <span className={clsx(
