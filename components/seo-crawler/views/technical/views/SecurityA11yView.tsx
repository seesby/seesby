...
const CARD = 'rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3';
    ...
    <div className="flex-1 overflow-auto custom-scrollbar p-3 grid grid-cols-12 gap-3">
      ...
      <div className={`${CARD} col-span-7`}>
        ...
        <BarChart
          data={d.a11yByImpact}
          x="impact" y="count"
          color={[STATUS_HEX.bad, STATUS_HEX.warn, STATUS_HEX.info, '#666']}
          height={180}
        />
      </div>
      ...
      <div className={`${CARD} col-span-5`}>
        ...
        />
        <div className="text-[10px] text-[#666] mt-1">% of pages with header set</div>
      </div>
      ...
      <div className={`${CARD} col-span-12`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-[10px] text-[#888] mb-1">A11y violations per page (sorted)</div>
            <Sparkline values={d.violationTrend} height={36} width={400} tone="warn" showArea />
          </div>
          <div>
            <div className="text-[10px] text-[#888] mb-1">Security issues per page (sorted)</div>
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody className="divide-y divide-[#111]">
                {d.failingPagesByIssue.map(issue => (
                  <tr key={issue.rule} className="hover:bg-[#0c0c0c]">
                    <td className="py-1.5 pr-3 text-[#ddd] max-w-[200px] truncate">{issue.rule}</td>
                    <td className="py-1.5 pr-3">
                      <ImpactBadge impact={issue.impact} />
                    </td>
                    <td className="py-1.5 pr-3 text-right font-mono text-white">{issue.count}</td>
                    <td className="py-1.5 text-[#888] max-w-[300px] truncate">
      ...
      {d.topRules.length > 0 && (
        <div className={`${CARD} col-span-12`}>
          ...
          <ul className="divide-y divide-[#111] text-[11px]">
            {d.topRules.map(([rule, count]) => (
              <li key={rule} className="flex justify-between py-1.5">
                <span className="text-[#ddd] truncate max-w-[400px]">{rule}</span>
                <span className="font-mono text-white shrink-0">{count as number}</span>
              </li>
            ))}
          </ul>
        </div>
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
          <span className="text-[11px] text-[#ccc] truncate">{item.label}</span>
          <span className="text-[10px] font-mono text-[#888] shrink-0 ml-2">{item.pass}/{total}</span>
        </div>
        <div className="h-1 rounded bg-[#0c0c0c] overflow-hidden">
...
function WcagBadge({ label, level }: { label: string; level: { total: number; pass: number; fail: number; passRate: number } }) {
  ...
  return (
    <div className="text-center">
      <div className="text-[10px] uppercase tracking-wider text-[#666] mb-1">Level {label}</div>
      <div className={clsx('text-[24px] font-mono font-bold', color)}>{level.passRate}%</div>
      <div className="text-[10px] text-[#888]">{level.pass}/{level.total} pass</div>
    </div>
  );
}
