    ...
    <div className="flex-1 overflow-auto custom-scrollbar p-3 grid grid-cols-12 gap-3 auto-rows-min">
      ...
      <div className={`${PANEL} col-span-12 lg:col-span-6`}>
        ...
        {ux.funnels.primary.length > 0 ? (
            ...
            {ux.funnels.primary.map((step: { label: string; value: number }, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2">
                  <div className="w-20 truncate text-[11px] text-[#ccc]">{step.label}</div>
                  ...
                  <div className="w-14 text-right text-[10px] font-mono text-[#ccc] tabular-nums">{fmtCompact(step.value)}</div>
                  {i > 0 && (
                    <div className={`w-12 text-right text-[9px] font-mono tabular-nums ${isWarning ? 'text-[#f59e0b]' : 'text-[#666]'}`}>
          ...
          <div className="text-[11px] text-[#555] py-4 text-center">No funnel data</div>
        )}
      </div>
          ...
          {(['low', 'medium', 'high'] as const).map(band => {
            ...
            return (
              <div key={band} className="flex-1">
                ...
                <div className="text-[9px] text-[#666] mt-1 text-center">{band} {count}</div>
              </div>
            );
          })}
        </div>
      ...
      <div className={`${PANEL} col-span-12 lg:col-span-6`}>
        ...
        {heatmaps.length > 0 ? (
          <div className="space-y-2">
            {heatmaps.slice(0, 3).map((h: any, i: number) => (
              <button
                ...
                className="w-full text-left flex items-center gap-2 py-1 hover:bg-[#111] rounded px-1 transition-colors"
              >
                {h.thumbUrl ? (
                  <img src={h.thumbUrl} alt="" className="w-10 h-7 object-cover rounded bg-[#111]" />
                ) : (
                  <div className="w-10 h-7 rounded bg-[#111] grid place-items-center text-[8px] text-[#555]">n/a</div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] text-[#ccc] truncate">{fmtUrl(h.pageUrl, 40)}</div>
          ...
          <div className="text-[11px] text-[#555] py-4 text-center">No heatmaps captured</div>
        )}
      </div>
      ...
      <div className={`${PANEL} col-span-12 lg:col-span-6`}>
        ...
        {experiments.length > 0 ? (
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {experiments.slice(0, 5).map((exp: any) => (
                <tr key={exp.id} className="border-t border-[#171717]">
                  <td className="py-1.5 text-[#ccc] truncate max-w-[140px]">{exp.name}</td>
                  <td className="text-center">
                    <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                      exp.status === 'running' ? 'bg-[#22c55e]' :
                      exp.status === 'won' ? 'bg-[#a78bfa]' :
                      exp.status === 'lost' ? 'bg-[#ef4444]' : 'bg-[#666]'
                    }`} />
                  </td>
                  <td className={`text-right font-mono tabular-nums ${
                    exp.uplift > 0 ? 'text-[#22c55e]' : exp.uplift < 0 ? 'text-[#ef4444]' : 'text-[#666]'
                  }`}>
          ...
          <div className="text-[11px] text-[#555] py-4 text-center">No experiments</div>
        )}
      </div>
      ...
      <div className={`${PANEL} col-span-12`}>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="text-[10px] text-[#888] ml-auto">
...
function KpiTile({ label, value, sub, delta, good, bad, warn, className = '' }: {
  ...
  const valueColor = good ? 'text-[#22c55e]' : bad ? 'text-[#ef4444]' : warn ? 'text-[#f59e0b]' : 'text-white';
  return (
    <div className={`${PANEL} ${className}`}>
      <div className="text-[9px] uppercase tracking-wider text-[#666]">{label}</div>
...
function FrictionRow({ label, value, bad }: { label: string; value: number; bad?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-[11px] font-mono tabular-nums ${bad ? 'text-[#ef4444]' : 'text-[#ccc]'}`}>{value}</span>
    </div>
  );
}
...
function CwvBar({ label, good, mid, poor }: { label: string; good: number; mid: number; poor: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-[#888] w-8">{label}</span>
