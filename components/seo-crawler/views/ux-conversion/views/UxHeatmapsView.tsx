...
export default function UxHeatmapsView() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Toolbar */}
      <div className="h-9 px-3 flex items-center justify-between border-b border-[#161616] bg-[#0a0a0a] shrink-0">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar p-3 grid grid-cols-3 gap-3">
        {filtered.map((h: any, i: number) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group rounded border border-[#1a1a1a] bg-[#0a0a0a] overflow-hidden hover:border-[#f43f5e]/30 transition-colors flex flex-col"
          >
            {h.thumbUrl ? (
              <img src={h.thumbUrl} alt="" className="block w-full aspect-[3/4] object-cover" />
            ) : (
              <div className="aspect-[3/4] grid place-items-center bg-[#111]">
                <span className="text-[11px] text-[#444]">No preview</span>
              </div>
            )}
            <div className="px-2.5 py-2 flex flex-col gap-1">
              <div className="text-[11px] text-white truncate">{h.pageUrl}</div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                    h.type === 'click' ? 'bg-[#f43f5e]' :
                    h.type === 'scroll' ? 'bg-[#3b82f6]' :
                    h.type === 'attention' ? 'bg-[#f59e0b]' : 'bg-[#888]'
                  }`} />
                  <span className="text-[10px] text-[#888] capitalize">{h.type}</span>
                </div>
      ...
      {options.map(k => (
        <button
          ...
          className={`h-6 px-2 rounded transition-colors ${
            value === k ? 'bg-[#171717] text-white' : 'text-[#888] hover:text-white hover:bg-[#121212]'
          }`}
        >
          {labels[k] ?? k}
        </button>
