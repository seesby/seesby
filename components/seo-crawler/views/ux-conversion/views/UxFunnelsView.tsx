...
export default function UxFunnelsView() {
  ...
  return (
    <div className="flex-1 grid grid-cols-12 gap-0 min-h-0">
      {/* Funnel list */}
      <div className="col-span-4 border-r border-[#161616] bg-[#0a0a0a] flex flex-col min-h-0">
        <div className="h-9 px-3 flex items-center border-b border-[#161616] shrink-0">
          <span className="text-[11px] text-[#666] uppercase tracking-wider">{funnels.length} funnels</span>
        </div>
        <div className="flex-1 overflow-auto custom-scrollbar">
          {funnels.length === 0 ? (
            <div className="p-4 text-[12px] text-[#666]">No funnels configured.</div>
          ) : (
            funnels.map((x: any) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[12px] text-white truncate">{x.name}</span>
        ...
        {f ? (
          <div className="p-4 space-y-4">
            ...
            <div className="rounded border border-[#1a1a1a] bg-[#0c0c0c] p-4">
              <div className="text-[10px] uppercase tracking-wider text-[#666] mb-3">{f.name}</div>
            ...
            {dropoffs.length > 0 && (
              <div className="rounded border border-[#1a1a1a] bg-[#0c0c0c] p-4">
                <div className="text-[10px] uppercase tracking-wider text-[#666] mb-3">Biggest drop-offs</div>
                <div className="space-y-2">
                  {dropoffs.slice(0, 5).map((d: any, i: number) => (
                    <div key={i} className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="text-[11px] text-[#ccc] truncate">{d.from} → {d.label}</div>
            ...
            {f.segments && f.segments.length > 0 && (
              <div className="rounded border border-[#1a1a1a] bg-[#0c0c0c] p-4">
                <div className="text-[10px] uppercase tracking-wider text-[#666] mb-3">Segments</div>
                <div className="grid grid-cols-2 gap-3">
                  {f.segments.map((s: any) => {
                    ...
                    return (
                      <div key={s.name} className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3">
                        <div className="text-[11px] text-white mb-2">{s.name}</div>
...
function MetricCard({ label, value, tone }: { label: string; value: string; tone?: string }) {
  const toneClass = tone === 'good' ? 'text-[#22c55e]' : tone === 'warn' ? 'text-[#f59e0b]' : tone === 'bad' ? 'text-[#ef4444]' : 'text-white';
  return (
    <div className="rounded border border-[#1a1a1a] bg-[#0c0c0c] px-3 py-2">
