    ...
    <div className="flex-1 grid grid-cols-12 gap-0 min-h-0">
      ...
      <div className="col-span-5 border-r border-[#161616] bg-[#0a0a0a] flex flex-col min-h-0">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          {list.length === 0 ? (
            <div className="p-4 text-[12px] text-[#666]">No replays captured.</div>
          ) : (
            list.map((r: any) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] text-white truncate flex-1">{r.pageUrl}</span>
                    <span className="text-[10px]">{DEVICE_ICON[r.device] || ''}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-1 text-[10px]">
                    <span className="text-[#888]">{duration}s</span>
                    {friction > 0 && (
                    ...
                    {r.source && r.source !== 'custom' && (
                      <span className="px-1.5 py-0.5 rounded bg-[#222] text-[#888] text-[9px]">{r.source}</span>
                    )}
                    <span className="text-[#666] ml-auto">{fmtDate(r.startedAt)}</span>
                  </div>
                </button>
              );
            })
      ...
      {/* Player area */}
      <div className="col-span-7 bg-[#0a0a0a] flex flex-col min-h-0">
        {selected ? (
          <>
            {/* Metadata bar */}
            <div className="h-9 px-3 flex items-center gap-4 border-b border-[#161616] shrink-0">
              <span className="text-[11px] text-[#666] truncate">{selected.pageUrl}</span>
              <div className="flex-1" />
              <span className="text-[10px] text-[#888]">{Math.round(selected.durationMs / 1000)}s</span>
              ...
              {ReplayPlayer ? (
                ...
                <div className="h-full grid place-items-center">
                  <div className="text-center">
                    <div className="w-16 h-16 rounded-full bg-[#111] grid place-items-center mx-auto mb-3">
                      <span className="text-[24px]">▶</span>
                    </div>
                    <div className="text-[13px] text-[#888] mb-1">Replay player</div>
                    <div className="text-[11px] text-[#555]">Connect a replay provider to watch sessions</div>
                  </div>
                </div>
              )}
            </div>
