      ...
      <div className="flex-1 min-h-0 overflow-auto custom-scrollbar">
        <div className="p-3 space-y-3">
          <div className="flex items-center gap-3 text-[11px]">
            <span className="text-[#888]">Feed source:</span>
            <span className="text-white">Google Merchant Center</span>
          ...
          <div className={`${CARD}`}>
            <div className="text-[10px] uppercase tracking-wider text-[#666] mb-3">Feed status summary</div>
            <div className="space-y-1.5">
              {statusSummary.map(s => (
                <div key={s.label} className="flex items-center gap-3 text-[11px]">
                  <span className="w-20 text-[#888]">{s.label}</span>
          ...
          {errorBreakdown.length > 0 && (
            <div className={`${CARD}`}>
              <div className="text-[10px] uppercase tracking-wider text-[#666] mb-2">Errors breakdown</div>
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-[#666] border-b border-[#171717]">
                ...
                <tbody>
                  {errorBreakdown.map(e => (
                    <tr key={e.type} className="border-b border-[#111]">
                      <td className="py-1.5 text-[#ccc]">{e.type}</td>
                      <td className="py-1.5 text-right font-mono text-white">{e.count}</td>
                      <td className="py-1.5 text-[#888] pl-4">{e.fix}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {parityRows.length > 0 && (
            <div className={`${CARD}`}>
              <div className="text-[10px] uppercase tracking-wider text-[#666] mb-2">Feed vs site parity</div>
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-[#666] border-b border-[#171717]">
                ...
                <tbody>
                  {parityRows.map(p => (
                    <tr key={p.sku} className="border-b border-[#111]">
                      <td className="py-1.5 text-[#ccc]">{p.sku}</td>
                      <td className="py-1.5 text-right font-mono text-white">${p.feedPrice.toFixed(0)}</td>
                      <td className="py-1.5 text-right font-mono text-white">${p.sitePrice.toFixed(0)}</td>
