    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className={`m-3 mb-0 ${BORDER} overflow-hidden`}>
        <div className="px-3 py-2 border-b border-[#161616] flex items-center justify-between">
          <div className="text-[10px] uppercase tracking-wider text-[#666]">Entity graph</div>
        ...
        <div className="h-[280px] relative">
          ...
          />
          {nodes.length === 0 && (
            <div className="absolute inset-0 grid place-items-center text-[12px] text-[#666]">
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden m-3 gap-3">
        ...
        <div className={`flex-1 flex flex-col min-w-0 ${BORDER} overflow-hidden`}>
          ...
          <div className="flex-1 overflow-auto custom-scrollbar">
            {/* Header */}
            <div className="flex items-center h-8 px-3 text-[10px] uppercase tracking-wider text-[#666] border-b border-[#161616] sticky top-0 bg-[#0a0a0a]">
            ...
            {entityRows.map(e => (
              <div
                ...
                className={`flex items-center h-8 px-3 text-[11px] border-b border-[#0f0f0f] cursor-pointer transition-colors ${
                  selectedEntity === e.id ? 'bg-[#0e0e0e]' : 'hover:bg-[#0c0c0c]'
                }`}
              >
                <div className="w-[160px] flex items-center gap-2 min-w-0">
                  <div
                    className="w-2 h-2 rounded-full shrink-0"
                    style={{ backgroundColor: typeColors[e.type] ?? '#666' }}
                  />
                  <span className="truncate text-white">{e.name}</span>
                </div>
                <div className="w-[80px] text-[#888]">{fmtNum(e.pageCount)}</div>
            ...
            {entityRows.length === 0 && (
              <div className="p-4 text-[12px] text-[#666] text-center">No entities found</div>
            )}
          </div>
        </div>
        ...
        <div className={`w-[280px] flex flex-col ${BORDER} overflow-hidden`}>
          <div className="px-3 py-2 border-b border-[#161616]">
            <div className="text-[10px] uppercase tracking-wider text-[#666]">Missing anchors for LLM extraction</div>
          </div>
              ...
              <div className="space-y-2">
                {missingAnchors.map((anchor, i) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <span className="text-[11px] text-[#ccc]">{anchor}</span>
                  </div>
                ))}
              </div>
            )}
