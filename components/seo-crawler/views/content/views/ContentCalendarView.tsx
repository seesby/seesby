...
const LABEL = 'text-[10px] uppercase tracking-wider text-[#666] mb-2 px-3 pt-2.5';
        ...
        <div className="flex items-center justify-between px-3 py-2">
            ...
            {(['month', 'week', 'day'] as ViewMode[]).map(v => (
              <button
                ...
                className={`h-[26px] px-3.5 text-[11px] rounded transition-colors ${
                  viewMode === v ? 'bg-[#1a1a1a] text-white' : 'text-[#666] hover:text-[#aaa]'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            {viewMode === 'month' && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="text-[11px] text-[#888] hover:text-white px-2 py-0.5 rounded hover:bg-[#171717]"
                >
                  &larr;
                </button>
                <span className="text-[12px] text-white font-medium w-[130px] text-center">
            ...
            <select
              ...
              className="h-[26px] px-2.5 text-[11px] bg-[#111] text-[#888] border border-[#1a1a1a] rounded outline-none cursor-pointer"
            >
              {sections.map(s => (
        ...
        <div className="flex items-center gap-3 px-3 pb-2 pt-1">
          {Object.entries(STATUS_DOT).map(([key, color]) => (
            <div key={key} className="flex items-center gap-1">
              ...
              <span className="text-[9px] text-[#555] capitalize">{key}</span>
            </div>
          ))}
          <div className="flex items-center gap-1.5 ml-2">
            ...
            <span className="text-[9px] text-[#555]">50-74</span>
        ...
        <div className="space-y-2 px-3 pb-3">
          {cadence.map(s => (
            <div key={s.name} className="flex items-center gap-2">
              <span className="text-[10px] text-[#888] w-[90px] truncate shrink-0">{s.name}</span>
              <div className="flex-1 h-[7px] bg-[#171717] rounded-full overflow-hidden">
                <div
              ...
              <span className="text-[9px] text-[#666] w-[48px] text-right shrink-0 tabular-nums">
          ...
          {cadence.length === 0 && (
            <div className="flex flex-col items-center justify-center py-6 gap-1">
              <div className="text-[11px] text-[#555]">No sections found</div>
              <div className="text-[9px] text-[#444]">Assign topic clusters to pages to see cadence data</div>
            </div>
          )}
        </div>
      </div>
    ...
    <div>
      <div className="grid grid-cols-7 border-b border-[#1a1a1a]">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
          <div key={d} className="text-[10px] uppercase tracking-wider text-[#555] text-center py-2 font-medium">{d}</div>
        ))}
      </div>
      <div className="grid grid-cols-7 bg-[#0a0a0a]">
        {Array.from({ length: data.weekdayOffset }).map((_, i) => (
          <div key={`pad-${i}`} className="min-h-[72px] bg-[#0d0d0d] border border-[#111] border-l-0 border-t-0 first:border-l first:border-t" />
        ))}
        {Array.from({ length: data.daysInMonth }).map((_, i) => {
          ...
          return (
            <button
              ...
              className={`relative min-h-[72px] p-2 text-left transition-colors border border-[#141414] border-l-0 border-t-0 ${
            ...
            >
              <span className={`text-[11px] tabular-nums ${
                isToday ? 'text-white font-semibold' : 'text-[#666]'
              }`}>{day}</span>
              {isToday && (
              ...
              {posts.length > 0 && (
                <>
                  <div className="text-[9px] text-[#888] mt-1">{posts.length} post{posts.length > 1 ? 's' : ''}</div>
                  <div className="flex gap-1 mt-1 flex-wrap">
                    ...
                    {posts.length > 6 && <span className="text-[8px] text-[#555] leading-none pt-0.5">+{posts.length - 6}</span>}
                  </div>
                </>
              )}
            </button>
        ...
        return (
          <div
            ...
            className={`rounded border min-h-[140px] p-2 ${
              isToday ? 'border-[#f59e0b]/30 bg-[#111]' : 'border-[#1a1a1a] bg-[#0a0a0a]'
            }`}
          >
            <div className="flex items-center justify-between mb-2">
              <span className={`text-[10px] ${isToday ? 'text-[#f59e0b] font-semibold' : 'text-[#666]'}`}>
                {date.toLocaleDateString('en-US', { weekday: 'short' })}
              </span>
              <span className={`text-[11px] tabular-nums ${isToday ? 'text-white font-medium' : 'text-[#555]'}`}>
                {date.getDate()}
              </span>
            </div>
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span
                      ...
                      style={{ background: qualityColor(p.qualityScore ?? 50) }}
                    />
                    <span className="text-[9px] text-[#ccc] truncate">{p.title || p.url.split('/').pop()}</span>
                  </div>
                </button>
              ))}
              {entries.length > 6 && (
                <span className="text-[8px] text-[#555] pl-3">+{entries.length - 6} more</span>
              )}
            </div>
          </div>
        );
...
function DayView({
  ...
  return (
    <div className="p-3">
      <div className="text-[12px] text-[#aaa] mb-3 pb-2 border-b border-[#1a1a1a]">
        {date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
        <span className="ml-2 text-[10px] text-[#666]">{entries.length} post{entries.length !== 1 ? 's' : ''}</span>
      </div>
      {entries.length > 0 ? (
        <div className="space-y-1">
          {entries.map((p: any) => (
            <button
              key={p.id}
              onClick={() => { onSelectPage(p.url); onOpenInspector(true); }}
              className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded hover:bg-[#111] transition-colors text-left"
            >
              <span
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[11px] text-[#ccc] truncate">{p.title || p.url}</div>
                <div className="text-[9px] text-[#666]">{p.cluster} · {p.author ?? '—'}</div>
              </div>
        ...
        <div className="flex flex-col items-center justify-center h-[120px] gap-1">
          <div className="text-[11px] text-[#555]">No posts on this day</div>
          <div className="text-[9px] text-[#444]">Select a day with published content</div>
        </div>
      )}
    </div>
  );
...
function CheckRow({ label, value, detail, ok }: { label: string; value: string; detail?: string; ok: boolean }) {
  return (
    <div className="flex flex-col gap-1 p-2 rounded bg-[#0d0d0d]">
      <span className="text-[10px] text-[#666]">{label}</span>
      <div className="flex items-center gap-1.5">
        <span className="text-[14px] text-[#ccc] font-mono tabular-nums font-medium">{value}</span>
        <span className={`text-[11px] ${ok ? 'text-[#22c55e]' : 'text-[#f59e0b]'}`}>
          {ok ? '\u2713' : '\u26A0'}
        </span>
      </div>
      {detail && <span className="text-[9px] text-[#555]">{detail}</span>}
    </div>
  );
}
