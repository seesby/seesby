...
export function intentColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      ...
      return <span className={`text-[10px] ${INTENT_COLORS[v] || 'text-[#666]'}`}>{v}</span>
    },
  }
}
...
export function positionColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      ...
      if (!v) return <span className="text-[10px] text-[#444]">&#8212;</span>
...
export function topKeywordColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      const v = ctx.getValue() as string
      return v
        ? <span className="text-[10px] text-[#888] truncate block">{v}</span>
        : <span className="text-[10px] text-[#444]">&#8212;</span>
    },
  }
}
...
export function canonicalColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      ...
      const isSelf = v === 'self'
      return (
        <span className={`text-[11px] truncate block ${isSelf ? 'text-[#888]' : STATUS.link}`}>
          {isSelf ? 'self' : v}
        </span>
      )
    },
...
export function redirectsColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      const v = ctx.getValue() as number
      return <span className={`font-mono text-[11px] ${v >= 2 ? STATUS.warn : 'text-[#aaa]'}`}>{v}</span>
    },
  }
}
...
export function certColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      ...
      if (v == null) return <span className="text-[#444]">&#8212;</span>
      return <span className={`font-mono text-[11px] ${v < 30 ? STATUS.bad : 'text-[#aaa]'}`}>{v}d</span>
    },
  }
}
...
export function a11yColumn<T extends Record<string, unknown>>(
  ...
  return {
    ...
    cell: (ctx) => {
      const v = ctx.getValue() as number
      if (v < 0) return <span className="text-[10px] text-[#666]">&#8212;</span>
