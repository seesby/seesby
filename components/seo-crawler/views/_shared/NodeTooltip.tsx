...
export function NodeTooltip({
  ...
  return (
    <div
      className="fixed z-50 pointer-events-none bg-[#111] border border-[#2a2a2a] rounded px-3 py-2 text-[11px] shadow-lg max-w-xs"
      style={{ left: x + 12, top: y - 8 }}
    >
      <div className="text-[#eee] font-medium truncate mb-1">{node.label || path}</div>
      <div className="text-[#666] truncate text-[10px] mb-1.5">{path}</div>
...
function Row({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span style={{ color: color ?? '#999' }}>{value}</span>
    </div>
  );
}
