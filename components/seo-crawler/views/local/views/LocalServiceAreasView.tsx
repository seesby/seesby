...
export default function LocalServiceAreasView() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-3 border-b border-[#1a1a1a] bg-[#0a0a0a] shrink-0">
        ...
        {grids.length > 0 && (
          <select className="h-7 px-2 text-[11px] bg-[#111] border border-[#222] text-[#ddd] rounded outline-none focus:border-[#F59E0B]"
                  value={selectedId ?? ''}
      ...
      <div className="flex-1 grid grid-cols-12 gap-3 p-3 min-h-0 overflow-auto custom-scrollbar">
        ...
        <div className="col-span-7 flex flex-col gap-3 min-h-0">
          <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3 flex flex-col">
          ...
          <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3 flex-1 min-h-0 relative">
            ...
            <div className="flex-1 min-h-0 relative">
              {selected && selected.grid && selected.grid.length > 0 ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                  <div className="text-[12px] text-[#666] mb-3">No grid scan yet. Run a grid scan from the right sidebar &gt; Pack tab.</div>
                  <button onClick={handleScan} className="h-7 px-3 text-[11px] bg-[#F59E0B] text-white rounded hover:bg-[#df3248]">
                    Run grid scan
                  </button>
                </div>
              )}
        ...
        <div className="col-span-5 flex flex-col gap-3 min-h-0">
          <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3 flex-1 min-h-0 overflow-auto custom-scrollbar">
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {coverageTable.map((row, i) => (
                    <tr key={row.territory} className={`border-b border-[#0f0f0f] ${i % 2 === 0 ? '' : 'bg-[#080808]'}`}>
                      <td className="py-1.5 pr-2 text-white truncate">{row.territory}</td>
                      <td className="py-1.5 px-2 text-right tabular-nums text-[#bbb]">{row.popCovered.toLocaleString()}</td>
                      <td className="py-1.5 px-2 text-right tabular-nums text-[#bbb]">{row.locations}</td>
                      <td className="py-1.5 px-2 text-right tabular-nums">
                        <span className={row.gapZips > 10 ? 'text-[#ef4444]' : row.gapZips > 5 ? 'text-[#f59e0b]' : 'text-[#bbb]'}>
                          {row.gapZips}
                        </span>
                      </td>
                      <td className="py-1.5 pl-2 text-right tabular-nums text-[#bbb]">{row.rankAvg}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <div className="text-[12px] text-[#666] py-8 text-center">No territory data available.</div>
            )}
          </div>

          <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3 max-h-[40%] overflow-auto custom-scrollbar">
            <SectionHead>Overlap &amp; Gap Warnings</SectionHead>
            {overlapWarnings.length > 0 ? (
              <div className="flex flex-col gap-1.5">
                {overlapWarnings.map((w, i) => (
                  <div key={i} className="flex items-start gap-2 text-[11px] py-1.5 px-2 rounded bg-[#0c0c0c] border border-[#171717]">
                    <span className="inline-block w-1.5 h-1.5 rounded-full mt-1.5 shrink-0" style={{ background: warnColor[w.type] }} />
                    <div className="flex-1 min-w-0">
                      <div className="text-[#ddd]">{w.message}</div>
              ...
              <div className="text-[12px] text-[#666] py-4 text-center">No warnings. Coverage looks clean.</div>
            )}
          </div>

          <div className="rounded border border-[#1a1a1a] bg-[#0a0a0a] p-3 text-[11px] text-[#888]">
...
function SectionHead({ children }: { children: React.ReactNode }) {
  return <div className="text-[10px] uppercase tracking-wider text-[#666] mb-2">{children}</div>;
}
