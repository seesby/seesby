...
export default function LocalReviewsView() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      {/* Filter chips */}
      <div className="shrink-0 border-b border-[#161616] bg-[#0a0a0a] px-3 py-1.5 flex items-center gap-2 text-[10px]">
            ...
            <button
              ...
              className={clsx(
                ...
                sourceFilter === s.id
                  ...
                  : 'bg-[#0c0c0c] text-[#888] border-[#1a1a1a] hover:text-[#ccc] hover:border-[#333]',
              )}
            >
              {s.label}
            </button>
      ...
      <div className="flex-1 grid grid-cols-12 gap-3 p-3 min-h-0 overflow-hidden">
        {/* Left: Review stream */}
        <ul className="col-span-5 rounded border border-[#1a1a1a] bg-[#0a0a0a] divide-y divide-[#171717] overflow-auto custom-scrollbar">
          {filtered.map((r: any) => (
            <li key={r.id} onClick={() => { setSelectedId(r.id); setDraft(r.reply ?? ''); }}
                className={clsx('px-3 py-2 cursor-pointer hover:bg-[#101010] transition-colors', selectedId === r.id && 'bg-[#0e0e0e]')}>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[#888] shrink-0">{fmtDate(r.createdAt)}</span>
                <span className="text-[11px] shrink-0">
                  {'\u2605'.repeat(Math.round(r.rating))}
                </span>
                <span className="text-[10px] text-white truncate">{r.author}</span>
                <span className="text-[9px] uppercase tracking-wider px-1 rounded bg-[#171717] text-[#bbb] shrink-0">{r.source}</span>
              </div>
              <div className="text-[11px] text-[#bbb] mt-1 line-clamp-2">{r.text}</div>
              <div className="flex items-center gap-2 mt-0.5 text-[10px]">
                <span className="text-[#666]">{r.locationId}</span>
          ...
          {filtered.length === 0 && (
            <div className="p-4 text-[12px] text-[#666]">No reviews match filters.</div>
          )}
        </ul>
        ...
        <div className="col-span-7 flex flex-col gap-3 min-h-0 overflow-auto custom-scrollbar">
          ...
          <div className={CARD}>
            ...
            {overTime.length > 1 ? (
              ...
              <div className="text-[12px] text-[#666] h-[160px] grid place-items-center">Need 2+ months of data.</div>
            )}
          </div>
                  ...
                  <div className="flex flex-col gap-1">
                    {topics.positive.map(t => (
                      <div key={t.topic} className="flex items-center gap-2 text-[11px]">
                        <span className="text-[#ddd] flex-1 truncate">{t.topic}</span>
                        <div className="w-16 h-1.5 rounded bg-[#171717] overflow-hidden">
                          <div className="h-full rounded bg-[#22c55e]" style={{ width: `${(t.count / topics.positive[0].count) * 100}%` }} />
                        </div>
                        <span className="text-[#888] tabular-nums w-5 text-right">{t.count}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-[11px] text-[#666]">No positive topics.</div>
                )}
              </div>
              <div>
                ...
                {topics.negative.length > 0 ? (
                  ...
                  <div className="text-[11px] text-[#666]">No negative topics.</div>
                )}
              </div>
            </div>
          </div>
          ...
          {selected && (
            <div className={CARD}>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[11px] text-white">{selected.author}</span>
                <span className="text-[11px] text-[#888]">{selected.rating}\u2605</span>
                <span className="text-[9px] uppercase tracking-wider px-1 rounded bg-[#171717] text-[#bbb]">{selected.source}</span>
                <span className="text-[10px] text-[#666] ml-auto">{fmtDate(selected.createdAt)}</span>
              </div>
              <p className="text-[12px] text-[#ddd] whitespace-pre-wrap mb-3">{selected.text}</p>
              <div className="text-[10px] uppercase text-[#666] mb-1">Reply</div>
              <textarea value={draft} onChange={e => setDraft(e.target.value)} rows={3}
                        className="w-full bg-[#0c0c0c] border border-[#1a1a1a] rounded p-2 text-[11px] text-white outline-none focus:border-[#f97316] custom-scrollbar" />
              <div className="flex gap-2 mt-2">
                <button onClick={() => replyToReview?.(selected.id, draft)}
                        className="h-6 px-3 text-[10px] bg-[#f97316] text-white rounded hover:bg-[#ea580c] transition-colors">
                  {selected.reply ? 'Update' : 'Send'}
                </button>
                <button onClick={() => setDraft('')}
                        className="h-6 px-3 text-[10px] text-[#999] border border-[#222] rounded hover:text-white transition-colors">
                  Clear
                </button>
              </div>
            </div>
