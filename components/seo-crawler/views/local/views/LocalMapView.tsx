  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 relative">
        {/* Floating controls */}
        <div className="absolute top-2 left-2 z-10 flex items-center gap-2 bg-[#0a0a0acc] backdrop-blur p-1.5 rounded border border-[#1a1a1a]">
          <span className="text-[10px] text-[#888] mr-1">Territory</span>
          <select
            value={territory}
          ...
          <span className="text-[10px] text-[#888] mr-1">Review age</span>
          <select
            value={reviewAge}
        ...
        {/* Floating stats */}
        <div className="absolute top-2 right-2 z-10 flex items-center gap-3 bg-[#0a0a0acc] backdrop-blur rounded border border-[#1a1a1a] px-3 py-1.5 text-[10px]">
          <span className="text-[#888]">{filteredMarkers.length} locations</span>
          <span className="text-[#333]">|</span>
          ...
          <span className="flex items-center gap-1">
            ...
            <span className="text-[#888]">Unverified</span>
          </span>
        </div>
      ...
      {/* Bottom panel */}
      <div className="shrink-0 h-[140px] border-t border-[#161616] bg-[#0a0a0a] p-3">
        <div className="grid grid-cols-2 gap-4 h-full">
          {/* Rating distribution */}
          <div className="flex flex-col min-h-0">
            <div className="text-[10px] uppercase tracking-wider text-[#666] mb-1 shrink-0">Rating distribution</div>
          ...
          <div className="flex flex-col min-h-0">
            <div className="text-[10px] uppercase tracking-wider text-[#666] mb-1 shrink-0">Pack presence by territory</div>
            <div className="flex-1 min-h-0 overflow-auto custom-scrollbar space-y-1.5">
              {packByTerritory.map(t => (
                <div key={t.territory} className="flex items-center gap-2 text-[11px]">
                  <span className="w-8 text-[#bbb] tabular-nums">{t.territory}</span>
                  <div className="flex-1 h-3 rounded bg-[#171717] overflow-hidden">
                    <div
                  ...
                  <span className="w-10 text-right text-[#bbb] tabular-nums">{t.pct}%</span>
                  <span className="w-10 text-right text-[#666] tabular-nums text-[10px]">{t.inPack}/{t.total}</span>
                </div>
              ))}
            </div>
          </div>
