...
export default function WqaStructureView() {
  ...
  return (
    <div className="flex-1 relative overflow-hidden">
          ...
          {(['tree', 'cluster', 'graph'] as Sub[]).map(k => (
            <button
              ...
              className={`h-6 px-2 text-[10px] uppercase tracking-wider rounded transition-colors ${
                sub === k ? 'bg-[#1a1a1a] text-white' : 'text-[#666] hover:text-[#999]'
              }`}
            >{k}</button>
          ))}
        </div>
      ...
      {sub === 'tree' && (
        <div className="absolute top-3 right-3 z-10 bg-[#0a0a0acc] backdrop-blur px-3 py-2 rounded border border-[#1a1a1a] text-[10px] flex items-center gap-4">
          <span className="text-[#888]">{visible.length}/{data.nodes.length} nodes</span>
          <span className="text-[#1a1a1a]">|</span>
          <span className="text-[#888]">Avg quality <span className="text-white font-mono">{data.stats.avgQuality}</span></span>
          <span className="text-[#888]">Avg depth <span className="text-white font-mono">{data.stats.avgDepth}</span></span>
          {data.stats.orphans > 0 && (
            <span className="text-[#888]">Orphans <span className="text-[#f59e0b] font-mono">{data.stats.orphans}</span></span>
          )}
          <span className="text-[#888]">{data.stats.clusterCount} categories</span>
        </div>
      )}
      ...
      {sub === 'tree' && (
        <div className="absolute bottom-3 left-3 z-10 flex items-center gap-3 bg-[#0a0a0acc] backdrop-blur p-2 rounded border border-[#1a1a1a] text-[10px]">
          ...
          {colorBy === 'cluster' && (
            <span className="text-[#888]">Colors represent page categories</span>
          )}
        </div>
      )}
