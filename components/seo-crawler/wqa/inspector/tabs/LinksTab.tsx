    ...
    <div className="space-y-3">
        ...
        <Card title="Internal">
          ...
          {inLinksList.length > 0 && (
            <div className="mt-2 pt-2 border-t border-[#141414]">
              <div className="text-[9px] text-[#444] uppercase tracking-widest mb-1.5">Top Sources</div>
              <div className="space-y-1 max-h-[160px] overflow-y-auto">
                {inLinksList.slice(0, 5).map((link, i) => (
                  <div key={`${link.url}-${i}`} className="flex items-center gap-2 text-[10px] py-1 border-b border-[#111] last:border-b-0">
                    <TruncatedUrl url={link.url} />
                    {link.anchor && (
                      <span className="text-[10px] text-[#555] shrink-0">anchor &ldquo;{String(link.anchor).slice(0, 30)}&rdquo;</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <Card title={`Third-party domains (${uniqueThirdPartyDomains.length})`}>
          <div className="flex flex-wrap gap-1.5">
            ...
            {uniqueThirdPartyDomains.length > 20 && (
              <span className="text-[10px] text-[#555]">+{uniqueThirdPartyDomains.length - 20} more</span>
            )}
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Card title="Co-citation (Peer Articles)">
          <div className="text-[9px] text-[#444] uppercase tracking-widest mb-1.5">Cited with</div>
          {coCitation.length > 0 ? (
            <div className="space-y-1">
              {coCitation.slice(0, 6).map((c: any, i: number) => (
                <div key={i} className="flex items-center justify-between py-1 border-b border-[#111] last:border-b-0">
                  <span className="text-[10px] text-[#ccc] truncate max-w-[160px]">{c.url || c}</span>
                  <span className="text-[10px] font-mono text-[#888]">{c.count || c.times || ''}x</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-[10px] text-[#444] py-2">No co-citation data</div>
          )}
        </Card>
      </div>
    </div>
