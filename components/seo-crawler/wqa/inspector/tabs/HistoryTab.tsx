...
function DeltaIcon({ current, prev }: { current: number; prev?: number }) {
  if (prev === undefined) return <Minus size={9} className="text-[#222]" />;
  ...
  return <Minus size={9} className="text-[#333]" />;
}

function SparklineCard({ label, values }: { label: string; values: number[] }) {
  return (
    <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2.5">
      <div className="text-[9px] text-[#444] uppercase tracking-widest mb-1.5">{label}</div>
  ...
  return (
    <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2.5">
      ...
      <div className="flex items-center gap-1.5 mt-1">
        <span className={`text-[14px] font-bold ${delta === 0 ? 'text-[#333]' : isGood ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
  ...
  if (!hasTrend && sessions.length <= 1 && events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[200px] text-center">
        ...
        <div className="text-[12px] text-[#555] font-medium">No trend data yet</div>
        <div className="text-[10px] text-[#333] mt-1">Run another crawl to see trends</div>
      </div>
    );
  }
        ...
        <Card title={`Sessions (${sessions.length})`}>
          <div className="space-y-1.5">
            {sessions.map((s: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between py-2 px-3 rounded-md bg-[#080808] border border-[#141414] hover:border-[#222] transition-colors">
                  <div className="flex items-center gap-2.5">
                    <div className="p-1.5 rounded-md bg-[#111]">
                      <History size={11} className="text-[#555]" />
                    </div>
                    <div>
                      <div className="text-[11px] text-white font-medium">{formatDate(s.date)}</div>
                      <div className="text-[9px] text-[#444] uppercase tracking-wider">{s.status || 'OK'}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-5">
                    {s.healthScore !== undefined && (
                      <div className="text-right min-w-[50px]">
                        ...
                        <div className="flex items-center gap-1 justify-end">
                          <span className="text-[11px] font-mono font-bold text-white">{s.healthScore}</span>
                    ...
                    {s.clicks !== undefined && (
                      <div className="text-right">
                        <div className="text-[9px] text-[#444] uppercase">Clicks</div>
                        <div className="flex items-center gap-1 justify-end">
                          <span className="text-[11px] font-mono font-bold text-white">{formatNumber(s.clicks)}</span>
        ...
        <Card title="Action Log">
          <div className="space-y-0">
            {actionLog.map((log: any, i: number) => (
              <div key={i} className="flex items-start gap-3 py-2 border-b border-[#111] last:border-b-0">
                <div className="shrink-0 mt-0.5">
                  <FileText size={10} className="text-[#555]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] text-[#ccc]">{log.action || log.label}</div>
                  <div className="text-[9px] text-[#444] mt-0.5">
                    {log.user && <span>{log.user} </span>}
                    {formatDate(log.date || log.timestamp)}
                    {log.outcome && <span className="text-[#555]"> &middot; {log.outcome}</span>}
                  </div>
                </div>
              </div>
            ))}
