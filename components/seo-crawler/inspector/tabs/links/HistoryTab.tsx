  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Card title="Changes detected">
          {!hasChanges && anchorChanges.length === 0 && relChanges.length === 0 && targetUrlChanges.length === 0 ? (
            <div className="text-[12px] text-[#666] italic">No changes detected.</div>
          ) : (
            <>
              {anchorChanged && (
                <div className="mb-2 pb-2 border-b border-[#141414]">
                  <div className="text-[11px] text-[#ccc] font-medium mb-1">Anchor changed {anchorChanges.length} time{anchorChanges.length > 1 ? 's' : ''}</div>
                  {anchorChanges.slice(0, 3).map((change: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="text-[10px] text-[#888] font-mono">
      ...
      <Card title="Source publishing cadence">
        {mentionsCount === 0 ? (
          <div className="text-[12px] text-[#666] italic">No publishing cadence data available.</div>
        ) : (
          <div className="text-[11px] text-[#ccc]">
            ...
            {cadenceTrend === 'stable' && <span className="text-[#888] ml-2">— stable</span>}
          </div>
        )}
      </Card>
          ...
          <div className="space-y-2">
            {history.map((s: any, i: number) => (
              <div key={i} className="flex items-center justify-between py-1 border-b border-[#111] last:border-b-0 text-[11px]">
                <span className="text-[#888]">{formatDate(s.date)}</span>
                <div className="flex items-center gap-4">
                  {s.inlinks != null && <span className="font-mono">{formatNumber(s.inlinks)} inlinks</span>}
                  {s.referringDomains != null && <span className="font-mono text-[#666]">{formatNumber(s.referringDomains)} ref domains</span>}
                  {s.backlinks != null && <span className="font-mono text-[#666]">{formatNumber(s.backlinks)} backlinks</span>}
                </div>
              </div>
            ))}
          </div>
