        ...
        <Card title={`In-links (${inLinksList.length})`}>
          <div className="space-y-1 max-h-[200px] overflow-y-auto custom-scrollbar">
            {inLinksList.map((link, i) => (
              <div key={`${link.url}-${i}`} className="flex items-center gap-2 text-[11px] py-1 border-b border-[#111] last:border-b-0">
                <TruncatedUrl url={link.url} />
                {link.anchor && (
                  <span className="text-[10px] text-[#555] shrink-0">"{String(link.anchor).slice(0, 40)}"</span>
                )}
              </div>
            ))}
          </div>
