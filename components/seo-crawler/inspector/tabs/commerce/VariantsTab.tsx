  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_160px] gap-3">
        {/* Variant table */}
        <div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2.5">Variants ({variantCount})</div>
          <div className="bg-[#060606] border border-[#1a1a1a] rounded-lg overflow-hidden">
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-[#1a1a1a]">
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Size</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Stock</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Price</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">GTIN</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Status</th>
                </tr>
              </thead>
              <tbody>
                {variants.slice(0, 30).map((v: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-[#111] bg-[#0a0a0a] hover:bg-[#141414]">
                      <td className="px-3 py-1.5 text-[#ccc]">{label}</td>
                      <td className="px-3 py-1.5 text-[#ccc]">{stockQty}</td>
                      <td className="px-3 py-1.5 text-[#ccc]">{price ? `$${Number(price).toFixed(2)}` : '—'}</td>
                      <td className="px-3 py-1.5 text-[#666] font-mono">{v.gtin || '—'}</td>
          ...
          <Card title="Options">
            {Object.entries(optionGroups).map(([key, values]) => (
              <div key={key} className="py-1">
                <div className="text-[11px] text-[#ccc]">{key} <span className="text-[#555]">{values.size} val{values.size !== 1 ? 's' : ''}</span></div>
              </div>
            ))}
          </Card>
