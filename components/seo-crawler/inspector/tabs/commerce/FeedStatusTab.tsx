            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-[#1a1a1a]">
                  ...
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Feed</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Site</th>
                  <th className="px-3 py-1.5 text-left text-[9px] text-[#444] uppercase tracking-widest font-bold">Match</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={i} className="border-b border-[#111] bg-[#0a0a0a] hover:bg-[#141414]">
                    <td className="px-3 py-1.5 text-[#ccc]">{row.field}</td>
                    <td className="px-3 py-1.5 text-[#ccc] truncate max-w-[100px]">{row.feed}</td>
                    <td className="px-3 py-1.5 text-[#ccc] truncate max-w-[100px]">{row.site}</td>
        ...
        <Card title="Disapprovals">
          {Array.isArray(disapprovals) && disapprovals.length > 0 ? (
            disapprovals.map((d: any, i: number) => (
              <div key={i} className="text-[11px] text-[#ccc] py-1">{typeof d === 'string' ? d : d.message || d.reason}</div>
            ))
          ) : (
        ...
        <Card title={`Warnings (${warnings.length})`}>
          {warnings.map((w: any, i: number) => (
            <div key={i} className="flex items-start gap-2 py-1">
              ...
              <span className="text-[11px] text-[#ccc]">{typeof w === 'string' ? w : w.message || w.field}</span>
            </div>
          ))}
        </Card>
      )}
