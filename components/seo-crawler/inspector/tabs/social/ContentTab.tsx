    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2 text-center">
          ...
          <div className="text-[9px] text-[#444] uppercase tracking-widest">Quality</div>
        </div>
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2 text-center">
          <div className="text-[12px] font-bold text-white">{formatNumber(topPosts.length)}</div>
          <div className="text-[9px] text-[#444] uppercase tracking-widest">Posts</div>
        </div>
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2 text-center">
          <div className="text-[12px] font-bold text-white">{formatNumber(topHashtags.length)}</div>
          <div className="text-[9px] text-[#444] uppercase tracking-widest">Hashtags</div>
        </div>
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-2 text-center">
          <div className="text-[12px] font-bold text-white">{formatNumber(postingFrequency)}</div>
          <div className="text-[9px] text-[#444] uppercase tracking-widest">Posts/Wk</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Card title="Post Types">
          {postTypes.length > 0 ? postTypes.map((pt: any, i: number) => {
            ...
            return (
              <div key={i} className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[12px] text-[#ccc]">{type}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-[#888]">{formatNumber(count)}</span>
                    <span className="text-[10px] text-[#555]">{formatPercent(engagement)}</span>
                  </div>
                </div>
            ...
            <div className="text-[11px] text-[#555]">No post type data</div>
          )}
        </Card>
        ...
        {contentThemes.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {contentThemes.map((theme: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center gap-1.5 bg-[#0a0a0a] border border-[#222] rounded px-2 py-1">
                  <span className="text-[11px] text-[#ccc]">{label}</span>
                  {score > 0 && <span className="text-[9px] text-[#555]">{formatNumber(score)}</span>}
                </div>
              );
            })}
          </div>
        ) : (
          <div className="text-[11px] text-[#555]">No theme data available</div>
        )}
      </Card>
        ...
        {topHashtags.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {topHashtags.slice(0, 15).map((tag: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center gap-1 bg-[#0a0a0a] border border-[#222] rounded px-2 py-1">
                  <span className="text-[11px] text-blue-400">#{name}</span>
                  {count > 0 && <span className="text-[9px] text-[#555]">{formatNumber(count)}</span>}
                </div>
              );
            })}
          </div>
        ) : (
          <div className="text-[11px] text-[#555]">No hashtag data available</div>
        )}
      </Card>
      ...
      <Card title="Top Posts">
        {topPosts.length > 0 ? topPosts.slice(0, 5).map((post: any, i: number) => (
          <div key={i} className="py-2 border-b border-[#141414] last:border-0">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-[#ccc] truncate max-w-[300px]">{post.text || post.content || post.title || `Post ${i + 1}`}</span>
          ...
          <div className="text-[11px] text-[#555]">No post data available</div>
        )}
      </Card>
    </div>
  );
