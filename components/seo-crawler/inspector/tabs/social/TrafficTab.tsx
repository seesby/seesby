    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Card title="Channel Breakdown">
          {trafficByChannel.length > 0 ? trafficByChannel.map((ch: any, i: number) => {
            ...
            return (
              <div key={i} className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[12px] text-[#ccc]">{channel}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-[#888]">{formatNumber(sessions)}</span>
                    <span className="text-[10px] text-[#555]">{pct.toFixed(0)}%</span>
                  </div>
                </div>
            ...
            <div className="text-[11px] text-[#555]">No channel data available</div>
          )}
        </Card>
      </div>
      ...
      <Card title="Top Landing Pages">
        {topLandingPages.length > 0 ? (
            ...
            {topLandingPages.slice(0, 8).map((lp: any, i: number) => {
              ...
              return (
                <div key={i} className="py-2 border-b border-[#141414] last:border-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-[10px] text-[#888]">
          ...
          <div className="text-[11px] text-[#555]">No landing page data available</div>
        )}
      </Card>
    </div>
  );
