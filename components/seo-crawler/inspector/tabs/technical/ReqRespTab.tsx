          ...
          <div className="space-y-1">
            {redirectChain.map((r: any, i: number) => {
              ...
              return (
                <div key={`${url}-${i}`} className="flex items-center gap-2 text-[11px] font-mono">
                  ...
                  <span className="text-[#888] truncate flex-1">{url}</span>
            ...
            {redirectChain.length === 0 && page?.redirectUrl && (
              <div className="flex items-center gap-2 text-[11px] font-mono">
                <span className="text-[#333] w-[20px] shrink-0">#1</span>
                <span className="text-[#888] truncate flex-1">{page.redirectUrl}</span>
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-[#1a1a1a]">
                  <th className="px-3 py-2 text-left text-[#444] uppercase tracking-widest text-[9px] font-bold">Name</th>
                  <th className="px-3 py-2 text-left text-[#444] uppercase tracking-widest text-[9px] font-bold">Domain</th>
                  <th className="px-3 py-2 text-left text-[#444] uppercase tracking-widest text-[9px] font-bold">Secure</th>
                  <th className="px-3 py-2 text-left text-[#444] uppercase tracking-widest text-[9px] font-bold">HttpOnly</th>
                </tr>
              </thead>
              <tbody>
                {cookies.map((c: any, i: number) => (
                  <tr key={i} className={`border-b border-[#111] ${i % 2 === 0 ? '' : 'bg-[#0a0a0a]'} hover:bg-[#141414]`}>
                    <td className="px-3 py-1.5 text-[#ccc] font-mono">{c?.name || '—'}</td>
                    <td className="px-3 py-1.5 text-[#888]">{c?.domain || '—'}</td>
