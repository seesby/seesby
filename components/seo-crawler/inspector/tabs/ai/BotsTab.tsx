              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="border-b border-[#1a1a1a]">
                    <th className="px-2 py-1.5 text-left text-[#555] uppercase tracking-widest font-bold">Bot</th>
                    <th className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">robots.txt</th>
                    <th className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">meta</th>
                    <th className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">x-robots</th>
                    <th className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">UA tested</th>
                    <th className="px-2 py-1.5 text-right text-[#555] uppercase tracking-widest font-bold">last hit</th>
                  </tr>
                </thead>
                <tbody>
                  {botEntries.map(bot => (
                    <tr key={bot.name} className="border-b border-[#111] hover:bg-[#111]">
                      <td className="px-2 py-1.5 text-[#ccc] font-mono whitespace-nowrap">{bot.name}</td>
                      ...
                      <td className="px-2 py-1.5 text-center">
                        {bot.xRobots !== undefined ? <CellCheck val={bot.xRobots} /> : <span className="text-[#555]">&mdash;</span>}
                      </td>
                      <td className="px-2 py-1.5 text-center">
                        {bot.uaTested !== undefined ? (
                          <span className={bot.statusCode === '200' ? 'text-[#22c55e]' : bot.statusCode ? 'text-[#ef4444]' : 'text-[#888]'}>
                            {bot.statusCode || '\u2014'}
                          </span>
                        ) : <span className="text-[#555]">&mdash;</span>}
                      </td>
                      <td className="px-2 py-1.5 text-right text-[#888] whitespace-nowrap">{bot.lastHit || '\u2014'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Card title="Rate limits">
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-[#666]">Crawl-delay</span>
                <span className="text-white">{crawlDelay || '\u2014'}</span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-[#666]">429 last 30d</span>
                <span className={count429 > 0 ? 'text-[#f59e0b]' : 'text-white'}>{count429}</span>
              </div>
            </div>
          </Card>
