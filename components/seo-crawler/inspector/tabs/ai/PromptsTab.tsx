...
function IntentBadge({ intent }: { intent: string }) {
  ...
  return (
    <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-medium ${colors[intent] || 'bg-[#1a1a1a] text-[#888]'}`}>
      {intent.slice(0, 3)}
    </span>
  );
}
    ...
    <div className="space-y-4">
      ...
      <Card title="Prompts triggering this page">
        {prompts.length > 0 ? (
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-[#1a1a1a]">
                  <th className="px-2 py-1.5 text-left text-[#555] uppercase tracking-widest font-bold">Prompt</th>
                  <th className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">Set</th>
                  {MODEL_KEYS.map(m => (
                    <th key={m} className="px-2 py-1.5 text-center text-[#555] uppercase tracking-widest font-bold">{m.slice(0, 1).toUpperCase()}</th>
                  ))}
                  <th className="px-2 py-1.5 text-right text-[#555] uppercase tracking-widest font-bold">Pos avg</th>
                </tr>
              </thead>
              <tbody>
                {prompts.map((prompt: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-[#111] hover:bg-[#111]">
                      <td className="px-2 py-1.5 text-[#ccc] max-w-[200px] truncate" title={q.query || q.keyword}>
                        {q.query || q.keyword}
                      </td>
          ...
          <div className="text-[11px] text-[#555] py-3">No prompt data available</div>
        )}
      </Card>
      ...
      {targetPrompts.length > 0 && (
        <Card title="Target prompts">
          <div className="text-[11px] text-[#888] mb-2">Prompts this page should win but doesn't:</div>
            ...
            {targetPrompts.map((target: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between py-1 text-[11px]">
                  <span className="text-[#ccc]">&ldquo;{q.query || q.keyword}&rdquo;</span>
