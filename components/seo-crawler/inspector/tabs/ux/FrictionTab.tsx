        ...
        <Card title="Signal Distribution">
          <div className="space-y-3">
            {breakdown.map((item) => {
              ...
              return (
                <div key={item.type} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[12px] text-[#ccc]">{item.type}</span>
                    <span className="text-[11px] text-[#888]">{formatNumber(item.count)} ({pct.toFixed(1)}%)</span>
                  </div>
                  ...
                  <div className="text-[10px] text-[#555] mt-1">{item.desc}</div>
                </div>
              );
            })}
          </div>
            ...
            {frictionByElement.slice(0, 10).map((el: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <span className="text-[12px] text-[#ccc] block truncate">{el.element || el.selector || el.label}</span>
                  {el.text && <span className="text-[10px] text-[#555] block truncate">{el.text}</span>}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[11px] text-[#888]">{formatNumber(el.count || el.clicks || 0)}</span>
                  <StatusBadge
        ...
        <Card title="Worst Pages">
          <div className="space-y-2">
            {worstPages.slice(0, 8).map((p: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <span className="text-[12px] text-[#ccc] block truncate">{p.title || p.url}</span>
                  {p.url && <span className="text-[10px] text-[#555] block truncate font-mono">{p.url}</span>}
                </div>
