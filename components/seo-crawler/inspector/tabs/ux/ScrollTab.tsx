        ...
        <Card title="Scroll Distribution">
          <div className="space-y-3">
            {[
              ...
              { label: 'None (0%)', value: none, color: 'bg-[#64748b]', textColor: 'text-[#888]' },
            ].map((item) => {
          ...
          <div className="space-y-2">
            {foldData.zones.slice(0, 8).map((z: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 flex items-center justify-between">
                <span className="text-[12px] text-[#ccc]">{z.label || z.element || `Zone ${i + 1}`}</span>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-[#888]">{Math.round(z.visibility || z.pct || 0)}%</span>
                  <StatusBadge
