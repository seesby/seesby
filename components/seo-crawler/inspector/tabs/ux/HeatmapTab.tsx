            ...
            {hotspots.slice(0, 10).map((h: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <span className="text-[12px] text-[#ccc] block truncate">{h.element || h.selector || `Zone ${i + 1}`}</span>
                  {h.text && <span className="text-[10px] text-[#555] block truncate">{h.text}</span>}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[11px] text-[#888]">{formatNumber(h.count || h.clicks || 0)} clicks</span>
        ...
        <Card title="Dead Zones">
          <div className="space-y-2">
            {deadZones.slice(0, 8).map((z: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 flex items-center justify-between">
                <span className="text-[12px] text-[#ccc]">{z.element || z.selector || `Zone ${i + 1}`}</span>
