    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5">
        ...
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Summary</div>
        ...
        {/* Distribution */}
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Distribution</div>
          {distribution ? (
              ...
              {[5, 4, 3, 2, 1].map(star => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2 text-[11px]">
                    <span className="text-[#666] w-6">{star}\u2605</span>
            ...
            <div className="text-[11px] text-[#555]">No distribution data</div>
          )}
        </div>

        {/* Recent */}
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Recent</div>
          <div className="space-y-1.5">
            {recentReviews.length > 0 ? recentReviews.slice(0, 5).map((r: any, i: number) => (
              <div key={i} className="flex items-start gap-1.5 text-[11px]">
                <span className="text-[#666] shrink-0">{r.daysAgo || r.date || ''}d</span>
                <span className="text-yellow-400 shrink-0">{'\u2605'.repeat(Math.min(r.rating || 0, 5))}</span>
                <span className="text-[#ccc] line-clamp-1 flex-1">{r.text || r.excerpt || ''}</span>
                <StatusBadge
              ...
              <div className="text-[11px] text-[#555]">No recent reviews</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5">
        {/* Sentiment */}
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Sentiment</div>
        ...
        {/* Topics */}
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Topics</div>
            ...
            {topics.length > 0 ? topics.slice(0, 8).map((t: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between py-[2px] text-[11px]">
                  ...
                  {count != null && <span className="text-[#666]">{count}</span>}
                </div>
              );
            }) : (
              <div className="text-[11px] text-[#555]">No topics extracted</div>
            )}
          </div>
        </div>
        ...
        {/* Sources */}
        <div className="bg-[#0e0e0e] border border-[#1a1a1a] rounded-lg p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-[#444] mb-2">Sources</div>
        ...
        <Card title={`Low Reviews Needing Attention (${lowReviews.length})`}>
          <div className="space-y-2">
            {lowReviews.slice(0, 5).map((review: any, i: number) => (
              <div key={i} className="bg-[#0a0a0a] border border-[#222] rounded px-3 py-2">
                <div className="flex items-center justify-between mb-1">
                  <StatusBadge status="fail" label={`${review.rating || '?'} stars`} />
                  <span className="text-[10px] text-[#666]">{review.date || ''}</span>
                </div>
                <div className="text-[11px] text-[#ccc] line-clamp-2">{review.text || review.content || review.comment || ''}</div>
              </div>
            ))}
          </div>
        </Card>
