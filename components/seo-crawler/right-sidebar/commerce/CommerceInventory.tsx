    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <Card padded={false}>
        <div className="px-3 py-2 border-b border-[#1f1f1f]">
          <span className="text-[11px] text-[#888]">OOS impact</span>
        </div>
      ...
      {s.inventory.oosTopList.length > 0 && (
        <Card padded={false}>
          <div className="px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#888]">OOS with most traffic</span>
          </div>
          <div className="flex flex-col border-t border-[#1f1f1f]">
            {s.inventory.oosTopList.map(p => (
              <button
                ...
                className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f] last:border-b-0 hover:bg-[#0f0f0f] transition-colors text-left w-full"
              >
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] text-[#ccc] truncate">{p.title}</div>
                  <div className="text-[10px] text-[#666]">{compactNum(p.sessions)} sessions · {p.backlinks} backlinks</div>
                </div>
      ...
      {s.byCategory.length > 1 && (
        <Card padded={false}>
          <div className="px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#888]">By category</span>
          </div>
          <div className="flex flex-col border-t border-[#1f1f1f]">
            {s.byCategory.slice(0, 8).map(c => (
              <div key={c.id} className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f] last:border-b-0">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-[#666]">{c.products} products · {fmtCurrency(c.revenue)} rev</div>
                </div>
      ...
      {s.topRevenue.length > 0 && (
        <Card padded={false}>
          <div className="px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#888]">Top products by revenue</span>
          </div>
          <div className="flex flex-col border-t border-[#1f1f1f]">
            {s.topRevenue.slice(0, 8).map(p => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] text-[#ccc] truncate">{p.title}</div>
                  <div className="text-[10px] text-[#666]">{fmtCurrency(p.revenue)} · {p.rating > 0 ? `${p.rating.toFixed(1)}★` : '—'} · {p.reviews} reviews</div>
                </div>
      ...
      <Card padded={false}>
        <div className="px-3 py-2 border-b border-[#1f1f1f]">
          <span className="text-[11px] text-[#888]">Pricing</span>
        </div>
        <div className="flex flex-col border-t border-[#1f1f1f]">
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#ccc]">Avg price</span>
            <span className="text-[11px] font-mono font-medium text-white">{fmtCurrency(s.avgPrice)}</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#ccc]">Price range</span>
            <span className="text-[11px] font-mono font-medium text-[#888]">{fmtCurrency(s.minPrice)} – {fmtCurrency(s.maxPrice)}</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#ccc]">Products on sale</span>
          ...
          <div className="flex items-center justify-between px-3 py-2">
            <span className="text-[11px] text-[#ccc]">vs benchmark AOV</span>
