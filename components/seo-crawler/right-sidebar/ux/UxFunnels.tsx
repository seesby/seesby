    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <Card title="Primary funnel">
        {s.funnels.primary.length > 0 ? (
            ...
            {s.funnels.primary.slice(0, 7).map((step, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2">
                  <span className="w-[70px] truncate text-[10px] text-[#888]">{step.label}</span>
                  <div className="flex-1 h-2.5 rounded-full bg-[#141414] overflow-hidden">
                    <div
                      className={`h-full rounded-full ${dropFromPrev > 50 ? 'bg-red-500/60' : dropFromPrev > 20 ? 'bg-amber-500/60' : 'bg-[#f43f5e]/60'}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <span className="w-[50px] text-right text-[10px] font-mono text-[#aaa]">{compactNum(step.value)}</span>
                  {dropFromPrev > 0 && (
                    <span className={`w-[36px] text-right text-[9px] font-mono ${dropFromPrev > 50 ? 'text-red-400' : dropFromPrev > 20 ? 'text-amber-400' : 'text-[#888]'}`}>
                      -{dropFromPrev}%
                    </span>
                  )}
                </div>
          ...
          <div className="text-[11px] text-[#666]">No funnel data. Set up funnels in the Funnels view.</div>
        )}
      </Card>
      ...
      <Card title="Biggest drops">
        {stepDrops.length > 0 ? (
          ...
          <div className="text-[11px] text-[#666]">No step drops detected.</div>
        )}
      </Card>
      ...
      <Card title="Secondary funnel">
        {s.funnels.secondary.length > 0 ? (
          ...
          <div className="text-[11px] text-[#666]">No secondary funnel data.</div>
        )}
      </Card>
      ...
      <Card title="All funnels">
        {s.funnels.list.length > 0 ? (
          ...
          <div className="text-[11px] text-[#666]">No funnels configured.</div>
        )}
      </Card>
      ...
      {bestFunnel && worstFunnel && (
        <div className="grid grid-cols-2 gap-2">
          <Card>
            <div className="text-[10px] uppercase tracking-wide text-[#666] mb-1">Best</div>
            <div className="text-[11px] text-white font-medium truncate">{bestFunnel.name}</div>
            <div className="text-[10px] text-emerald-400">{fmtPct(bestFunnel.completion * 100)}</div>
          </Card>
          <Card>
            <div className="text-[10px] uppercase tracking-wide text-[#666] mb-1">Worst</div>
            <div className="text-[11px] text-white font-medium truncate">{worstFunnel.name}</div>
        ...
        <Card tone="accent">
          <div className="flex items-center gap-2">
            <div className="text-[11px] text-[#ccc]">
              <span className="font-medium text-white">{worstStep.name}</span> has the biggest drop
            </div>
      ...
      <Card title="Completion trend">
        {hasTrend && s.funnels.completionSeries.length > 1 ? (
          ...
          <div className="text-[11px] text-[#666]">Trend data will appear after the next crawl.</div>
        )}
      </Card>
    </div>
  )
