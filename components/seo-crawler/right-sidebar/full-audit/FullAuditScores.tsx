    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <Card>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-wide text-[#888]">Overall score</div>
        ...
        <ul className="flex flex-col gap-2.5">
          {PILLARS.map((p) => (
            <li key={p.key} className="flex items-center gap-2">
              <span className="w-20 shrink-0 text-[11px] text-[#888]">{p.label}</span>
              <div className="relative h-2 flex-1 overflow-hidden rounded-full bg-[#141414]">
                <div
                  className="absolute inset-y-0 left-0 rounded-full transition-all duration-500"
                  style={{ width: `${pillars[p.key]}%`, background: p.color }}
                />
              </div>
              <span className="w-8 text-right text-[11px] tabular-nums text-[#ccc]">{pillars[p.key]}</span>
            </li>
          ))}
        </ul>
      </Card>
      ...
      {cohort && (
        <Card title={`Cohort: ${cohort.label}`}>
          ...
          <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#888]">
            <span>Percentile <span className="text-[#ccc] tabular-nums">{cohort.percentile}</span></span>
