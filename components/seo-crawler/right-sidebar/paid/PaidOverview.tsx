    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <Card>
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-wide text-[#888]">Paid score</div>
            ...
            {perfRows.map((r) => (
              <div key={r.label} className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f] last:border-b-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-[#666]">{r.desc}</span>
                </div>
                <span className="text-[11px] font-mono font-medium text-white">{r.value}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card padded={false}>
        <div className="flex flex-col border-t border-[#1f1f1f]">
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#888]">Avg Quality Score</span>
            <span className="text-[11px] font-mono font-medium text-white">
              {s.avgQs > 0 ? s.avgQs.toFixed(1) : '—'} /10
            </span>
          </div>
          <div className="flex items-center justify-between px-3 py-2 border-b border-[#1f1f1f]">
            <span className="text-[11px] text-[#888]">QS &ge; 8 (excellent)</span>
            <span className="text-[11px] font-mono font-medium text-[#22c55e]">{s.above8}</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2">
            <span className="text-[11px] text-[#888]">Total campaigns</span>
            <span className="text-[11px] font-mono font-medium text-white">{s.total}</span>
          </div>
        </div>
      </Card>
